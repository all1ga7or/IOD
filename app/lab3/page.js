    ...
    <div className="container animate-fade">
      ...
      {activeTab === 'input' && (
        <div className="animate-fade">
          ...
          <h2 className="section-title">📊 Множинні порівняння та Компроміс</h2>
          
          <div className="glass-card" style={{ padding: '24px', marginBottom: '24px', overflowX: 'auto' }}>
            <h3 style={{ color: 'var(--accent)', marginBottom: '16px' }}>Ранги за множинним порівнянням</h3>
            <table style={{ borderCollapse: 'collapse', fontSize: '0.72rem', minWidth: '100%' }}>
              <thead>
                <tr>
                  <th style={{ padding: '4px 6px', borderBottom: '1px solid var(--border-color)', color: 'var(--text-muted)', textAlign: 'left' }}>Об'єкт \ Експерт</th>
                  {inputSummary.surveyData.map((_, j) => (
                    <th key={j} style={{ padding: '4px 6px', borderBottom: '1px solid var(--border-color)', color: 'var(--teal)' }}>{j + 1}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {multipleComparisonMatrix && multipleComparisonMatrix.map((row, i) => (
                  <tr key={i}>
                    <td style={{ padding: '4px 6px', color: 'var(--teal)', fontWeight: 600, whiteSpace: 'nowrap' }}>{objNames[i]}</td>
                    {row.map((val, j) => (
                      <td key={j} style={{ padding: '4px 6px', textAlign: 'center', color: val > 0 ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                        {val}
                      </td>
                    ))}
                  </tr>
          ...
          {compromiseMatrixData && (
            <div className="glass-card" style={{ padding: '24px', overflowX: 'auto' }}>
              <h3 style={{ color: 'var(--accent)', marginBottom: '16px' }}>Матриця компромісів</h3>
              <table style={{ borderCollapse: 'collapse', fontSize: '0.72rem', minWidth: '100%' }}>
                <thead>
                  <tr>
                    <th style={{ padding: '4px 6px', borderBottom: '1px solid var(--border-color)', color: 'var(--text-muted)', textAlign: 'left' }}>Компроміс (Ранг)</th>
                    {inputSummary.surveyData.map((_, j) => (
                      <th key={j} style={{ padding: '4px 6px', borderBottom: '1px solid var(--border-color)', color: 'var(--teal)' }}>{j + 1}</th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {compromiseMatrixData.matrix.map((row, i) => (
                    <tr key={i}>
                      <td style={{ padding: '4px 6px', color: 'var(--teal)', fontWeight: 600, whiteSpace: 'nowrap' }}>
                        <span style={{ color: 'var(--text-primary)', marginRight: '6px' }}>{objNames[i]}</span>
                        <span style={{ color: 'var(--text-muted)' }}>({compromiseMatrixData.compRanks[i]})</span>
                      </td>
                      {row.map((val, j) => (
                        <td key={j} style={{ padding: '4px 6px', textAlign: 'center', color: val !== null ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                          {val !== null ? val : ''}
                        </td>
                      ))}
                    </tr>
                  ...
                  <tr>
                    <td style={{ padding: '4px 6px', color: 'var(--accent)', fontWeight: 600, borderTop: '2px solid var(--border-color)' }}>Сума відстаней</td>
                    {compromiseMatrixData.sums.map((sum, j) => (
                      <td key={j} style={{ padding: '4px 6px', textAlign: 'center', fontWeight: 600, borderTop: '2px solid var(--border-color)' }}>{sum}</td>
                    ))}
                  </tr>
                  <tr>
                    <td style={{ padding: '4px 6px', color: 'var(--accent)', fontWeight: 600 }}>Задоволеність (%)</td>
                    {compromiseMatrixData.satisfaction.map((sat, j) => (
                      <td key={j} style={{ 
                        padding: '4px 6px', textAlign: 'center', fontWeight: 600,
                        color: sat > 70 ? 'var(--teal)' : sat > 40 ? '#ffc107' : '#cf6679'
                      }}>{sat}%</td>
                    ))}
                  </tr>
                </tbody>
