...
async function pg_getLab2Results() {
  ...
  // Отримуємо топ-14 об'єктів з результатів ЛР1 (якщо даних мало, використовуємо дефолтний SUBSET)
  ...
  if (lab1Res.scores && lab1Res.scores.some(s => parseInt(s.total_score) > 0)) {
    // Відбираємо топ-14
    currentSubset = lab1Res.scores.slice(0, 14).map(o => ({ id: parseInt(o.id), name: o.name }));
  }
...
function computeLab3(objects, votes, expertNames, allObjects = null) {
  ...
  const multipleComparison = objects.map(obj => {
    return votes.map(v => {
      const pick = v.picks.find(p => p.objectId === obj.id || parseInt(p.objectId) === parseInt(obj.id));
      return pick ? pick.rank : 0;
    });
  });

  // --- Compromise Matrix Data ---
  const compRankMap = {};
  finalRanking.forEach((name, i) => compRankMap[name] = i + 1);

  const compMatrix = objects.map(obj => {
    const cr = compRankMap[obj.name];
    return votes.map(v => {
      const pick = v.picks.find(p => p.objectId === obj.id || parseInt(p.objectId) === parseInt(obj.id));
      if (pick && cr !== undefined) {
        return Math.abs(cr - pick.rank);
      }
      return null;
    });
  });

  const compSums = votes.map((_, col) => {
    let sum = 0;
    for (let row = 0; row < objects.length; row++) {
      if (compMatrix[row][col] !== null) sum += compMatrix[row][col];
    }
    return sum;
  });

  const maxD = (objects.length - 3) * 3;
  const compSatisfaction = compSums.map(s => {
    return maxD > 0 ? parseFloat(Math.max(0, (1 - s / maxD) * 100).toFixed(1)) : 100;
  });

  const compromiseMatrixData = {
    matrix: compMatrix,
    compRanks: objects.map(obj => compRankMap[obj.name] || '-'),
    sums: compSums,
    satisfaction: compSatisfaction
  };
  ...
  return {
    ...
    multipleComparisonMatrix: multipleComparison,
    compromiseMatrixData: compromiseMatrixData,
    binaryMatrix,
    objNames,
    totalPerms,
    bestSumD,
