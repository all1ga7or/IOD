...
function computeLab3(objects, votes, expertNames) {
...
async function pg_getLab3Results() {
  ...
  if (lab1Res.scores && lab1Res.scores.length > 0) {
    candidateObjects = lab1Res.scores
      .slice(0, 10)
      .map(o => ({ id: parseInt(o.id), name: o.name }));
  } else {
    candidateObjects = OBJECTS_SEED.slice(0, 10).map((name, idx) => ({ id: idx + 1, name }));
  }

  const result = computeLab3(candidateObjects, votes, expertNames);
  return result;
}
...
export async function getLab3Results() {
  ...
  const objects = OBJECTS_SEED.slice(0, 10).map((name, idx) => ({ id: idx + 1, name }));
  ...
  return computeLab3(objects, votes, expertNames);
}
...
function computeLab4(objects, votes, expertNames, lab3Data) {
  const inputSummary = {
    objects: objects.map(o => o.name),
    experts: expertNames,
    surveyData: votes.map((v, i) => ({
...
async function pg_getLab4Results() {
  ...
  return computeLab4(candidateObjects, votes, expertNames, lab3Data);
}

export async function getLab4Results() {
  if (isPostgresMode()) return pg_getLab4Results();
  const objects = OBJECTS_SEED.slice(0, 8).map((name, idx) => ({ id: idx + 1, name }));
  ...
  return computeLab4(objects, votes, expertNames, { bestSumD: null, cookSumMedians: [] });
}
